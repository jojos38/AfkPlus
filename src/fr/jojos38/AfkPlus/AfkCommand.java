...
public class AfkCommand implements CommandExecutor {
	...
	public void toggleAfk(Player player) { // Toggle Afk for specific player
    	if (plugin.afkList.contains(player.getUniqueId())) { // If player is Afk
	...
	public boolean onCommand(CommandSender sender, Command command, String msg, String[] args) {
        if (sender instanceof Player) {
        	...
        	else if (commandName.equals("afkplusreload")) {
        		if (sender.hasPermission("afkplus.reload")) { 
	        		plugin.reloadConfig();
	        		plugin.loadConfig();
	        		player.sendMessage("AfkPlus configuration reloaded");
        		}
        	}
        }
        return true;
